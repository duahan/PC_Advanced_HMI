...
#End Region
    ...
    Public Sub SubscribeTo(ByVal PLCAddress As String, ByVal numberOfElements As Integer, ByVal callBack As EventHandler(Of SubscriptionHandlerEventArgs))
        SubscribeTo(PLCAddress, numberOfElements, callBack, "")
    End Sub

    Public Sub SubscribeTo(ByVal PLCAddress As String, ByVal numberOfElements As Integer, ByVal callBack As EventHandler(Of SubscriptionHandlerEventArgs), ByVal propertyName As String)
            ...
            For Each Subscript In m_SubscriptionList
                If Not Subscript.SuccessfullySubscribed Then
                    ...
                    If Not String.IsNullOrWhiteSpace(address.Trim) Then
                        Try
                            Dim NotificationID As Integer = m_CommComponent.Subscribe(address, CShort(Subscript.NumberOfElements), 250, AddressOf SubscribedDataReturned)
            ...
            If (p(i) IsNot Nothing) AndAlso (Not String.IsNullOrEmpty(p(i).Name)) AndAlso ((p(i).PropertyType) Is GetType(String)) Then ' (String.Compare(p(i).Name, "Container") <> 0) Then
                ...
                If p(i).Name.IndexOf("PLCAddress", StringComparison.CurrentCultureIgnoreCase) = 0 Then
                    ...
                    If Not (String.IsNullOrEmpty(PLCAddress)) Then
                        ...
                        If j < p.Length Then
                            SubscribeTo(PLCAddress, 1, Nothing, PropertyToWrite)
                        End If
                    End If
                End If
            End If
