'**********************************************************************************************
'* Multi-State Selector for AdvancedHMI
'*
'* Archie Jacobs
...
Public Class MultiStateSelector
    Inherits MfgControl.AdvancedHMI.Controls.MultiStateIndicator
...
#Region "Events"
    ...
    Private Sub MomentaryButton_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseDown
        ...
        If (m_PLCAddressClick IsNot Nothing AndAlso (String.Compare(m_PLCAddressClick, "") <> 0)) And Enabled AndAlso m_CommComponent IsNot Nothing Then
            Try
                Dim ButtonWidth As Integer = Me.Width / Me.States.Count
                Dim ButtonClicked As Integer = Math.Floor(e.X / ButtonWidth)
                ButtonClicked = Math.Min(States.Count - 1, ButtonClicked)
                m_CommComponent.Write(m_PLCAddressClick, States(ButtonClicked).Value)
    ...
    Private Sub MomentaryButton_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseUp
        'If (m_PLCAddressClick IsNot Nothing AndAlso (String.Compare(m_PLCAddressClick, "") <> 0)) And Enabled Then
        '    Try
    ...
    Protected Overrides Sub OnCreateControl()
        ...
        If Me.DesignMode Then
            ...
            If m_CommComponent Is Nothing Then
                Me.Parent.Site.Container.Add(New AdvancedHMIDrivers.EthernetIPforSLCMicroCom)
                m_CommComponent = CType(Me.Parent.Site.Container.Components(Me.Parent.Site.Container.Components.Count - 1), MfgControl.AdvancedHMI.Drivers.IComComponent)
            End If
        Else
            SubscribeToCommDriver()
        End If
