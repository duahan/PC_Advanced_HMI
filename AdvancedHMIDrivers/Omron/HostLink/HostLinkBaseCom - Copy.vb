...
#Region "Subscription"
        ...
        Private Function SortPolledAddresses(ByVal A1 As MfgControl.AdvancedHMI.Drivers.Omron.PolledAddressInfo, ByVal A2 As MfgControl.AdvancedHMI.Drivers.Omron.PolledAddressInfo) As Integer
            If (A1.Address.MemoryAreaCode > A2.Address.MemoryAreaCode) Or (A1.Address.MemoryAreaCode = A2.Address.MemoryAreaCode And A1.Address.ElementNumber > A2.Address.ElementNumber) _
                Or (A1.Address.MemoryAreaCode = A2.Address.MemoryAreaCode And A1.Address.BitNumber > A2.Address.BitNumber) Then
                Return 1
        ...
        Private Sub PollUpdate(ByVal so As Object)
            ...
            While i < SubscriptionList.Count And i < ItemCountToUpdate
                ...
                If i < ItemCountToUpdate AndAlso (PollCounts / SubscriptionList(i).PollRateDivisor) = Convert.ToInt32(PollCounts / SubscriptionList(i).PollRateDivisor) Then
                    Dim SavedAsync As Boolean = m_AsyncMode
                    Dim ElementsToRead As Integer = 1
                    Try
                        m_AsyncMode = True
                        ...
                        While (i + j + 1) < ItemCountToUpdate AndAlso _
                            ...
                            ((SubscriptionList(i + j + 1).Address.ElementNumber + SubscriptionList(i + j + 1).Address.NumberOfElements) - SubscriptionList(i).Address.ElementNumber) < 25 AndAlso _
                            SubscriptionList(i + j).Address.BitsPerElement = SubscriptionList(i + j + 1).Address.BitsPerElement
                    ...
                    Try
                        ElementsToRead = ElementSpan + 1
                        If SubscriptionList(i).Address.BitsPerElement = 1 Then
                            SubscriptionList(i).Address.NumberOfElements += SubscriptionList(i).Address.BitNumber
                            SubscriptionList(i).Address.NumberOfElements = 16
                        End If
                        Dim ReadAddress As New MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress(SubscriptionList(i).Address.Address, ElementsToRead)
                        ReadAddress.Tag = SubscriptionList(i).ID
                        ReadAddress.InternalRequest = True

                        Try
                            CurrentTNS = Convert.ToInt32(Read(ReadAddress))
                        Catch ex1 As Exception
                            Dim fw As New System.IO.StreamWriter(".\DriverErrorLog.log", True)
...
#Region "Public Methods"
        ...
        Public Function Read(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress) As Integer
            ...
            '* Save this 
            CurrentTNS = CByte(GetNextTransactionNumber(255))
        ...
        Public Function BeginWrite(ByVal startAddress As String, ByVal numberOfElements As Integer, ByVal dataToWrite() As String) As Integer Implements IComComponent.BeginWrite
            Dim address As New MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress(startAddress, numberOfElements)

            Return WriteData(address, dataToWrite)
        End Function


        Public Function WriteData(ByVal address As String, ByVal dataToWrite() As String) As Integer
        ...
        '* Memory Area Read
        '* Reference : Section 4-3
        Public Function WriteData(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress, ByVal dataToWrite() As String) As Integer
        ...
        Public Function ReadProgram() As Byte()
            ...
            '*************************************************************
            '* Save this 
            Dim CurrentTNS As UInt16 = CByte(GetNextTransactionNumber(255))
        ...
        Public Function WriteProgram(ByVal data() As Byte) As Integer
            ...
            '*************************************************************
            '* Save this 
            Dim CurrentTNS As Byte = CByte(GetNextTransactionNumber(255))
...
#Region "Private Methods"
        ...
        Protected Sub DataLinkLayerDataReceived(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
            ...
            '* 9-MAR-12 V1.15
            If Requests(e.TransactionNumber) Is Nothing OrElse Requests(e.TransactionNumber).Response Is Nothing _
                OrElse (e.TransactionNumber < 0 Or e.TransactionNumber > (Requests.Length - 1)) Then
                Exit Sub
            End If

            e.PlcAddress = Requests(e.TransactionNumber).Address

            If Not Requests(e.TransactionNumber).IsWrite Then
                '* Extract the data
                Dim values() As String = ExtractData(HostFrame.EncapsulatedData, 0, Requests(e.TransactionNumber))
                For i As Integer = 0 To values.Length - 1
                    e.Values.Add(values(i))
                Next

                '* Verify that enough elements were returned before continuing V1.11 6-MAR-12
                If e.Values.Count < Requests(e.TransactionNumber).NumberOfElements Then
                    Exit Sub
                End If
        ...
        Private Sub SendToSubscriptions(ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
            ...
            Dim sid As UShort = e.TransactionNumber ' Fins.Header.ServiceId
            While i < SubscriptionList.Count
