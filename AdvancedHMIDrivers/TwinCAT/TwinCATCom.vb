...
Public Class TwinCATCom
    ...
    Private GroupedSubscriptionReads As New System.Collections.Generic.List(Of System.Collections.Generic.List(Of MfgControl.AdvancedHMI.Drivers.TcAdsSymbolReadWrite))
            ...
            If value.IndexOf(".ini", 0, StringComparison.CurrentCultureIgnoreCase) > 0 Then
                Try
                    If Not Me.DesignMode Then
                        ...
                        NewIP = p.GetSetting("TARGETIPADDRESS")
                    End If
                    m_IPIniFile = value
    ...
    Private Sub CreateDLLInstance()
        ...
        SyncLock (CreateDLLLockObject)
            ...
            If (Not DLL.ContainsKey(MyDLLInstance) OrElse (DLL(MyDLLInstance) Is Nothing)) Then
                Dim NewDLL As New MfgControl.AdvancedHMI.Drivers.ADSforTwinCATEx
                ...
                If System.Net.IPAddress.TryParse(m_TargetIPAddress, ip) Then
                    NewDLL.TargetIPAddress = m_TargetIPAddress
                Else
                    ...
                    Catch ex As Exception
                        ...
                        Exit Sub
                    End Try
                    NewDLL.TargetIPAddress = ipe.AddressList(0).ToString
                End If

                NewDLL.ServerNetID = m_TargetAMSNetID
                NewDLL.ServerPort = m_TargetAMSPort
                NewDLL.UserName = m_UserName
                NewDLL.Password = m_Password
                NewDLL.UseStaticRoute = m_UseStaticRoute
...
#Region "Subscribing"
    ...
    Public Function Subscribe(ByVal plcAddress As String, ByVal numberOfElements As Int16, ByVal pollRate As Integer, ByVal callback As EventHandler(Of MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)) As Integer Implements MfgControl.AdvancedHMI.Drivers.IComComponent.Subscribe
        ...
        SyncLock (SubscribeLock)
            ...
            ''*********************************
            'Try
            '    If DLL.ContainsKey(MyDLLInstance) AndAlso DLL(MyDLLInstance) IsNot Nothing And String.Compare(m_TargetAMSNetID, "0.0.0.0.0.0") <> 0 Then
            '        Sym = GetSymbolInfo(plcAddress)
            '    Else
            '        Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException(-3, "DLL Layer not yet created")
            '    End If
            'Catch ex As MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException
            '    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException(ex.ErrorCode, "ADS Error " & ex.ErrorCode & " - " & plcAddress & " " & MfgControl.AdvancedHMI.Drivers.ADSUtilities.DecodeMessage(ex.ErrorCode))
            'End Try
            ...
            '    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException(-70, "Failed to get symbol")
            'End If

            '***********************************************************
            '* Check if there was already a subscription made for this
            '***********************************************************
            ' Dim Sym As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo = UsedSymbols(i)
            ...
            If (Not SubExists) Then
                ...
                'GroupedSubscriptionReads.Add(tmpPA)
    ...
    Private Sub SubscriptionUpdate()
        ...
        While Not StopSubscriptions
            ...
            ElseIf GroupedSubscriptionReads.Count <= 0 Or m_DisableSubscriptions Then
    ...
    Private Sub CreateGroupedReadList()
        ...
        SyncLock (SubscribeLock)
            ...
            While SubIndex < SubscriptionList.Count
                Try
                    If SubscriptionList(SubIndex).Symbol.IndexGroup <= 0 Then
                        Dim sym As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo = GetSymbolInfo(SubscriptionList(SubIndex).Symbol.Name)

                        If sym IsNot Nothing Then
                            Dim tmpSI As New SubscriptionInfo

                            tmpSI.PollRate = SubscriptionList(SubIndex).PollRate
                            tmpSI.dlgCallBack = SubscriptionList(SubIndex).dlgCallBack
                            tmpSI.ID = SubscriptionList(SubIndex).ID
                            tmpSI.Symbol = sym
                            tmpSI.NumberOfElements = Convert.ToInt32(SubscriptionList(SubIndex).NumberOfElements)

                            SubscriptionList.RemoveAt(SubIndex)
                            SubscriptionList.Insert(SubIndex, tmpSI)
                        Else
                ...
                Catch ex As MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException
                    ...
                    m_SynchronizingObject.BeginInvoke(SubscriptionList(SubIndex).dlgCallBack, x)
                    FailedToGetSymbol = True
                Catch ex As Exception
                    Dim dbg = 0
                    FailedToGetSymbol = True
            ...
            SubscriptionList.Sort(AddressOf SortPolledAddresses)
            If FailedToGetSymbol Then SubscriptionListChanges += 1
        End SyncLock
        ...
        While SubIndex < SubscriptionList.Count
            NumberToRead = 1
            Try
                While (SubIndex + NumberToRead) < SubscriptionList.Count AndAlso _
                    (SubscriptionList(SubIndex).Symbol.IndexGroup < &HF000) AndAlso _
                      (SubscriptionList(SubIndex).Symbol.IndexGroup = SubscriptionList(SubIndex + NumberToRead).Symbol.IndexGroup) AndAlso _
                      (SubscriptionList(SubIndex + NumberToRead).Symbol.IndexOffset - SubscriptionList(SubIndex).Symbol.IndexOffset) <= 20
                    NumberToRead += 1
                End While
            Catch ex As Exception
                Dim nr = 0
            ...
            BytesToRead = Convert.ToInt32((SubscriptionList(SubIndex + NumberToRead - 1).Symbol.IndexOffset + SubscriptionList(SubIndex + NumberToRead - 1).Symbol.Size) _
                                               - SubscriptionList(SubIndex).Symbol.IndexOffset)


            Gr = New SubscriptionInfo
            Gr.Symbol = New MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo
            Gr.Symbol.IndexGroup = SubscriptionList(SubIndex).Symbol.IndexGroup
            Gr.Symbol.IndexOffset = SubscriptionList(SubIndex).Symbol.IndexOffset
            Gr.Symbol.DataType = SubscriptionList(SubIndex).Symbol.DataType
        ...
        While i < CombinedSubscriptionReads.Count
            ...
            Try
                ...
                Dim itemGroup As New System.Collections.Generic.List(Of MfgControl.AdvancedHMI.Drivers.TcAdsSymbolReadWrite)
    ...
    Private Sub SendToSubscriptions(ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
        ...
        While Index < SubscriptionList.Count
            For s = 0 To Requests(TNSByte).SymbolInfo.Count - 1
                If SubscriptionList(Index).Symbol.IndexGroup = Requests(TNSByte).SymbolInfo(s).IndexGroup Then
                    Dim ByteIndexOffset As Long = SubscriptionList(Index).Symbol.IndexOffset
                    Dim HighestRequiredByteIndex As Long = SubscriptionList(Index).Symbol.IndexOffset + (SubscriptionList(Index).NumberOfElements * SubscriptionList(Index).Symbol.Size)
                    ...
                    If (HighestRequiredByteIndex <= HighestByteIndexReturned) _
                                 AndAlso (SubscriptionList(Index).Symbol.IndexOffset >= Requests(TNSByte).SymbolInfo(s).IndexOffset) Then
                        ...
                        Try
                            f.PlcAddress = SubscriptionList(Index).Symbol.Name
                            f.SubscriptionID = SubscriptionList(Index).ID
                            f.Values = New System.Collections.ObjectModel.Collection(Of String)
                            'Try
                            Sym = GetSymbolInfo(SubscriptionList(Index).Symbol.Name)
                            'Catch ex As Exception
                            'Dim gsi = 0
                            ' End Try
                            Try
                                v = ExtractData(Requests(TNSByte).SymbolInfo(s).RawData, Convert.ToInt32(ByteIndexOffset - Requests(TNSByte).SymbolInfo(s).IndexOffset), Convert.ToInt32(SubscriptionList(Index).NumberOfElements), Sym)
                                For i As Integer = 0 To Convert.ToInt32(SubscriptionList(Index).NumberOfElements - 1)
                                    f.Values.Add(v(i))
                                Next
                            Catch ex As Exception
                                Dim vv = 0
                        ...
                        If m_SynchronizingObject IsNot Nothing Then
                            ...
                            m_SynchronizingObject.BeginInvoke(SubscriptionList(Index).dlgCallBack, x)
                        Else
...
#Region "Public Methods"
        ...
        If WaitForResponse(TIDByte, 3000) = 0 Then
            ...
            If Requests(TIDByte).Response IsNot Nothing Then
                If Requests(TIDByte).Response.ErrorId = 0 Then
                    ...
                    Return tmp
                Else
                    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException(Requests(TIDByte).Response.ErrorId, MfgControl.AdvancedHMI.Drivers.ModbusUtilities.DecodeMessage(Requests(TIDByte).Response.ErrorId))
                End If
            Else
    ...
    Public Function ReadGroup(ByVal itemGroup As System.Collections.Generic.List(Of MfgControl.AdvancedHMI.Drivers.TcAdsSymbolReadWrite)) As Integer
        Dim TransactionID As Integer
        TransactionID = GetNextTransactionID(32767)
        Dim TIDByte As Integer = (TransactionID And 255)
...
#Region "Private Methods"
    ...
    Private Function ExtractData(ByVal dataIN As System.Collections.ObjectModel.Collection(Of Byte), ByVal startIndex As Integer, ByVal elementCount As UInt32, ByVal symbol As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo) As System.Collections.ObjectModel.Collection(Of String)
    ...
    Private Function ExtractData(ByVal dataIN As List(Of Byte), ByVal startIndex As Integer, ByVal elementCount As UInt32, ByVal symbol As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo) As System.Collections.ObjectModel.Collection(Of String)
        Dim data(dataIN.Count - startIndex - 1) As Byte
        For index = 0 To data.Length - 1
            data(index) = dataIN(index + startIndex)
        Next
    ...
    Private Function ExtractData(ByVal dataIN As System.Collections.ObjectModel.Collection(Of String), ByVal startIndex As Integer, ByVal elementCount As Integer, ByVal symbol As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo) As System.Collections.ObjectModel.Collection(Of String)
    ...
    Private Function ExtractData(ByVal dataIN() As Byte, ByVal startIndex As Integer, ByVal elementCount As Integer, ByVal symbol As MfgControl.AdvancedHMI.Drivers.TcAdsSymbolInfo) As System.Collections.ObjectModel.Collection(Of String)
    ...
    Private Function GetNextTransactionID(ByVal max As Integer) As Integer
        Return DLL(MyDLLInstance).GetInvokeID
    End Function
    ...
    Private Sub DataLinkLayerDataReceived(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
        If e.OwnerObjectID = MyObjectID Then
            Try
                ...
                If Requests(TIDByte) Is Nothing OrElse Requests(TIDByte).Response IsNot Nothing Then
                    Exit Sub
                End If
                            ...
                            If Requests(TIDByte) IsNot Nothing Then
                                ...
                                Else
                                    ...
                                    For i = 0 To Requests(TIDByte).SymbolInfo.Count - 1
                                        ...
                                        Requests(TIDByte).SymbolInfo(i).ReadResult = BitConverter.ToUInt32(Resp.EncapsulatedData.ToArray, i * 4)
